...
3. Deploy backend through `.github/workflows/backend-deploy.yml`, selecting `api`, `worker` or `both` for manual runs. On push, the workflow detects API and worker path changes and deploys only the affected backend components. The API Lambda is packaged with `apps/api/scripts/build_lambda.sh`, which emits a function zip for `morphix_api` code and a dependencies zip attached as a Lambda Layer; the worker remains a Docker/ECR image because it runs on ECS Fargate.
